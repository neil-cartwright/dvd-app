<div class="left-side p-2">
  ...
  <div class="bg-gray-200 m-2 p-2 rounded flex items-center">
    ...
    <span class="ml-auto sans" id="current-search-count"
      >&nbsp;{{ $films->count() }}
      @if($films->count() > 1) films @else film @endif
    </span>
    @endif
  </div>

  <div
    class="bg-gray-200 m-2 p-2 rounded flex justify-between items-center cursor-pointer hover:bg-gray-300"
    id="show-genres"
  >
    <a class="">Genres</a>
    <i
      class="fas fa-caret-square-down fa-2x ml-2 text-green-500"
      id="genre-caret"
    ></i>
  </div>
  ...
  <div
    id="genres"
    class="invisible bg-gray-200 m-2 p-2 rounded flex items-center flex-wrap"
  >
    <a
      class="p-2 m-1 rounded border-solid border-gray-500 bg-green-500 hover:bg-green-600 text-white"
      href="/"
      >All films</a
    >
    @foreach($genres as $genre)
    <a
      class="sans p-2 m-1 rounded border-solid border-gray-500  text-center flex-grow bg-gray-300 hover:bg-gray-400
         @if( request()->segment(count(request()->segments())) == $genre )
            bg-teal-400 font-semibold text-white
         @endif
         "
      href="/films/search/{{ $genre }}"
      >{{ $genre }}</a
    >
    @endforeach
  </div>
  ...
  <div
    class="login-section @if(!$errors->any()) invisible @endif bg-gray-200 m-2 p-2 rounded flex flex-col"
  >
    <p
      for="login"
      class="mr-4 font-bold mb-4 flex justify-between items-center"
    >
      @if(Auth::check())
    ...
    <form action="/login" method="POST" class="flex-grow flex flex-col">
      ...
      <input
        type="submit"
        value="Login"
        class="w-1/3 self-end bg-blue-600 text-white rounded p-2 mt-8"
      />
    </form>
  </div>
  @endif
