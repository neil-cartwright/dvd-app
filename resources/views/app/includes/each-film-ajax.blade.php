<div class="p-6 flex flex-col items-around border-b border-1 border-solid border-gray-400 sans">

	<div class="title flex items-center justify-between py-2">

		<div>
			<span
				class="slab film-title -ml-2 px-2 bg-gray-200 text-sm md:text-lg font-bold mr-2 tracking-wider">${$film.title}</span>
			<span class="year hover:underline text-xs">
				<a href="/films/search/year/${$film.year}">${$film.year}</a></span></div>

		@if(Auth::check())
		<div class="flex flex-col md:flex-row"><a class="ml-auto delete-film cursor-pointer text-sm" data-id="${$film.id}"
				data-title="${$film.title}">
				Delete
			</a>
			<a class="ml-4 text-sm" href="/films/edit/${$film.id}">Update</a></div>
		@endif
	</div>

	<div class="credits flex flex-wrap mt-2">
		<span class="text-sm md:text-base font-semibold mr-2">
		...
		<div class="flex flex-grow items center justify-start sm:justify-start">
			...
			<span class="text-sm italic mr-2">
				<a class="hover:underline" href="/films/search/supporting_actor/${$film.supporting_actor}">
